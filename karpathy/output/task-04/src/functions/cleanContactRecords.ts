...
import {
  CleanContactRecordsOptions,
  CleanedContactRecord,
  RawContactRecord,
} from "../types/contact";
 ...
 */
function normalizePhone(input: unknown): { value: string; error: string | null } {
 ...
 */
function normalizeEmail(input: unknown): { value: string; error: string | null } {
  if (input === null || input === undefined) {
 ...
 */
export function cleanContactRecords(
  records: unknown,
  options: CleanContactRecordsOptions = {},
): CleanedContactRecord[] {
  ...
  for (let i = 0; i < records.length; i++) {
    const raw = records[i] as RawContactRecord | null | undefined;
  ...
  const out: CleanedContactRecord[] = new Array(records.length);
  for (let i = 0; i < records.length; i++) {
...
function buildCleanedRecord(
  ...
  try {
    ...
    const isDuplicate =
      emailError === null && email !== "" && (emailCounts.get(email) ?? 0) > 1;

    // Carry signup_date verbatim if it's a string; null otherwise.
    const signupDate =
      typeof record.signup_date === "string" ? record.signup_date : null;

    // Preserve unknown extra fields (anything not in RESERVED_KEYS).
    const cleaned: CleanedContactRecord = {
      phone,
      email,
    ...
    if (raw !== null && raw !== undefined && typeof raw === "object") {
      for (const key of Object.keys(raw as Record<string, unknown>)) {
        if (!RESERVED_KEYS.has(key)) {
          cleaned[key] = (raw as Record<string, unknown>)[key];
        }
      }
    }
    if (!isValid) {
