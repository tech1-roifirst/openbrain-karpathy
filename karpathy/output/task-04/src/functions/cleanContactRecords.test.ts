...
import { cleanContactRecords } from "./cleanContactRecords";
import type { RawContactRecord } from "../types/contact";
...
describe("cleanContactRecords — integration", () => {
  ...
  test("integration #2: invalid email is flagged with validation_errors", () => {
    const out = cleanContactRecords(
      [{ phone: "555-123-4567", email: "not-an-email", signup_date: "2026-01-16" }],
      { logger: silent },
    );
...
describe("cleanContactRecords — edge cases", () => {
  test("edge #1: null phone is handled gracefully and marked invalid", () => {
    const out = cleanContactRecords(
      [{ phone: null, email: "ok@example.com" }],
      { logger: silent },
    );
    expect(out[0].is_valid).toBe(false);
  ...
  test("edge #3: whitespace phone is trimmed and normalized", () => {
    const out = cleanContactRecords(
      [{ phone: "(555) 123-4567 ", email: "ok@example.com" }],
      { logger: silent },
    );
  ...
  test("edge #5: email with plus tag is accepted", () => {
    const out = cleanContactRecords(
      [{ phone: "5551234567", email: "john+tag@example.com" }],
      { logger: silent },
    );
  ...
  test("edge #8: 1000 rows complete in well under 5 seconds", () => {
    ...
    const out = cleanContactRecords(rows, { logger: silent });
...
describe("cleanContactRecords — robustness", () => {
  test("non-array input returns empty array (does not throw)", () => {
    expect(cleanContactRecords(null, { logger: silent })).toEqual([]);
    expect(cleanContactRecords(undefined, { logger: silent })).toEqual([]);
    expect(cleanContactRecords("nope" as unknown, { logger: silent })).toEqual([]);
    expect(cleanContactRecords({} as unknown, { logger: silent })).toEqual([]);
  });

  test("non-object record entries are normalized to invalid records", () => {
    const out = cleanContactRecords(
      [null, undefined, 42, "x", { phone: "5551234567", email: "ok@x.com" }] as unknown as RawContactRecord[],
      { logger: silent },
    );
  ...
  test("idempotent: feeding output back in produces the same canonical fields", () => {
    const first = cleanContactRecords(
      [{ phone: "(555) 123-4567", email: "John@Example.com", signup_date: "2026-01-15" }],
      { logger: silent },
    );
    const second = cleanContactRecords(first as unknown as RawContactRecord[], {
      logger: silent,
    });
  ...
  test("custom logger receives invalid-record diagnostics", () => {
    ...
    cleanContactRecords([{ phone: "x", email: "y" }], {
      logger: (msg, ctx) => {
        calls.push({ msg, ctx });
      },
    });
    ...
    expect(Array.isArray((calls[0].ctx as { errors: unknown }).errors)).toBe(true);
  });
});
